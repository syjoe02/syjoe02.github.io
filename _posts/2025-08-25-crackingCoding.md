...
## 6) Unicode with Normalization (Most accurate)

- Unicode encodings may differ → Use `normalization` to check uniqueness

    é → a single code point (U+00E9)
	
    e + ´ → two code points (U+0065 + U+0301)

- when to Use? : When visual correctness with Unicode (e.g emojis, accented character)

```java
import java.text.Normalizer;
import java.util.*;

public static boolean isUniqueUnicodeNormalized(String s) {
    if (s == null) return true;
